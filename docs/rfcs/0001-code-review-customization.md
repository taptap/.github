...
- **E：`paths_skip` 实现为 pre-process diff 的硬过滤**（在喂给 Claude 前用 git diff filter 物理剔除路径）——能提供 100% 排除保证，但实现成本大（需要决定关联文件改动如何处理），且当前没有"必须真硬过滤"的实际需求（详见 §4.2）。延后到具体场景出现时再评估。
